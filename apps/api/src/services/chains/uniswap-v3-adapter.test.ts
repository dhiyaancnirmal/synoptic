...
test("uniswap adapter in test runtime quotes and executes deterministically", async () => {
  ...
  assert.equal(quote.source, "DIRECT_VIEM");
  ...
  assert.equal(swap.source, "DIRECT_VIEM");
});
